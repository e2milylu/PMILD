...
class GaussianCopula():
    ...
    def __init__(self, training_mode='standard', tol=0.01, max_iter=50, 
                 random_state=101, n_jobs=1, verbose=0, num_ord_updates=1, 
                 min_ord_ratio=0.1, stepsize_func=lambda k, c=5:c/(k+c), 
                 const_stepsize=0.5, num_pass=2, batch_size=100, window_size=200, 
                 decay=None, realtime_marginal=True, corr_diff_type=['F']):
    ...
    def fit_offline(self, X, first_fit=True, max_iter=None, convergence_verbose=True):
        ...
        ----------
            See Parameters of fit()
        '''
        #转换数据格式为numpy，判断变量类型是连续的还是有序类型
        X = self._preprocess_data(X)
        # do marginal estimation
        # for every fit, a brand new marginal transformation is used 
        if first_fit:
            cdf_types, inverse_cdf_types = self.get_cdf_estimation_type(p = X.shape[1])
            self.transform_function = TransformFunction(X, #数组
                                                        cont_indices=self._cont_indices, #self._cont_indices是布尔类型
                                                        ord_indices=self._ord_indices, #布尔类型
                                                        cdf_types=cdf_types, #字符串类型
                                                        inverse_cdf_types=inverse_cdf_types #字符串类型
                                                       )
    ...
    def _em_step(self, Z, r_lower, r_upper):
        ...
        if max_workers ==1:
            ...
            res_dict = _latent_operation_body_(args) #对传入的参数 args 进行解引用（dereference）
    ...
    def _init_copula_corr(self, Z):
        ...
        self._corr = np.corrcoef(Z_imp, rowvar=False) #衡量变量间的线性相关性
        if self._verbose > 1:
    ...
    def get_vartype_indices(self, X):
        ...
        def is_special_type(i,x):
            ...
            x = x[obs]
            n = len(x)
            uniques, counts = np.unique(x, return_counts=True) #unique默认是升序排序
            if len(counts) == 1:
                print("index,uniques, counts", i, uniques, counts)
        ...
        for i, col in enumerate(X.T):
            is_ord, is_lower_truncated, is_upper_truncated, is_twoside_truncated = is_special_type(i,col)
...
def PMILD_imputation(mask_df):
    ...
    imputed_np = gc.fit_transform(mask_df)
    return imputed_np
